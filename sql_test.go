...
func TestDatabaseSQLiteKV(t *testing.T) {
	...
	// Put: key is empty
	err = db.Set(&apiserver.KV{})
	assert.Error(t, err)

	// Put: value is empty
	err = db.Set(&apiserver.KV{Key: []byte("baetyl")})
	assert.NoError(t, err)
	...
	kv1 := &apiserver.KV{Key: []byte("/k/1"), Value: []byte("/k/1/data")}
	kv2 := &apiserver.KV{Key: []byte("/k/2"), Value: []byte("/k/2/data")}
	kv3 := &apiserver.KV{Key: []byte("/s/3"), Value: []byte("/d/3/data")}
	...
	// test Chinese
	kvc1 := &apiserver.KV{Key: []byte("/陈/张"), Value: []byte("/陈/张里")}
	kvc2 := &apiserver.KV{Key: []byte("/陈/王"), Value: []byte("/陈/王里")}
	kvc3 := &apiserver.KV{Key: []byte("/李/王"), Value: []byte("/李/王里")}
...
func BenchmarkDatabaseSQLite(b *testing.B) {
	...
	b.Run("put", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			...
			db.Set(&apiserver.KV{Key: key, Value: key})
		}
	})
