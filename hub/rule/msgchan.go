...
func (c *msgchan) putQ0(msg *common.Message) {
	if !c.msgtomb.Alive() {
		c.log.WithError(errMsgChanClosed).Error("failed to put message (qos=0)")
		return
	}
	select {
	case c.msgq0 <- msg:
...
func (c *msgchan) putQ1(msg *common.Message) {
	...
	case <-c.msgtomb.Dying():
		c.log.WithError(errMsgChanClosed).Error("failed to put message (qos=1)")
	case c.msgq1 <- msg:
	}
}
...
func (c *msgchan) goProcessingQ0() error {
	c.log.Debug("task of processing message from channel (Q0) begins")
	defer c.log.Debug("task of processing message from channel (Q0) stopped")
loop:
	for {
		select {
		case <-c.msgtomb.Dying():
...
func (c *msgchan) goProcessingQ1() error {
	c.log.Debug("task of processing message from channel (Q1) begins")
	defer c.log.Debug("task of processing message from channel (Q1) stopped")
loop:
	for {
		select {
		case <-c.msgtomb.Dying():
...
func (c *msgchan) goWaitingAck() error {
	c.log.Debug("task of waiting acknowledge begins")
	defer c.log.Debug("task of waiting acknowledge stopped")
	for {
		select {
		case <-c.acktomb.Dying():
			return nil
