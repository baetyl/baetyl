...
func TestMqttTcp(t *testing.T) {
	...
	m, err := NewManager(addr, trans.Certificate{}, func(conn transport.Conn) {
		p, err := conn.Receive()
	...
	dailer, err := mqtt.NewDialer(trans.Certificate{})
...
func TestMqttTcpTls(t *testing.T) {
	...
	dailer, err := mqtt.NewDialer(trans.Certificate{Insecure: true})
	assert.NoError(t, err)
	conn, err := dailer.Dial(url)
	assert.NoError(t, err)
	err = conn.Send(request, false)
...
func TestServerException(t *testing.T) {
	...
	_, err := NewManager(addr, trans.Certificate{}, nil)
	switch err.Error() {
