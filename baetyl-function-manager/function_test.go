...
func TestFunction_CallAsync(t *testing.T) {
	...
	err = f.CallAsync(in, func(in, out *baetyl.FunctionMessage, err error) {
		assert.NoError(t, err)
		assert.Equal(t, in, out)
		close(finish)
	})
...
func TestFunctionPerf_CallAsync(t *testing.T) {
	...
	queue := make(chan *baetyl.FunctionMessage, 100)
	in := &baetyl.FunctionMessage{
		ID:               1,
		QOS:              1,
		Topic:            "delay",
	...
	cb := func(in, out *baetyl.FunctionMessage, err error) {
		queue <- out
	}
	count := 1000
...
func (c *mockContext) Config() *baetyl.ServiceConfig {
	return nil
}
func (c *mockContext) UpdateSystem(file string, clean bool) error {
	return nil
}
func (c *mockContext) InspectSystem() (*baetyl.Inspect, error) {
	return nil, nil
}
...
func (i *mockInstance) Call(msg *baetyl.FunctionMessage) (*baetyl.FunctionMessage, error) {
	if msg.Topic == "delay" {
