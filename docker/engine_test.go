...
func TestRun(t *testing.T) {
	...
	defer cli.Close()
	stats := mockStats{services: map[string]baetyl.InstancesStats{}}
	e := &dockerEngine{
		InfoStats: &stats,
		cli:       cli,
...
type mockStats struct {
	...
	services map[string]baetyl.InstancesStats
	sync.Mutex
}
...
func (s *mockStats) SetInstanceStats(serviceName, instanceName string, partialStats baetyl.PartialStats, persist bool) {
	s.Lock()
	defer s.Unlock()
	service, ok := s.services[serviceName]
	if !ok {
		service = baetyl.InstancesStats{}
