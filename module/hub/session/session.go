...
func (s *session) send(p packet.Generic, async bool) error {
	...
	if err != nil {
		return errors.Annotate(err, "Send message failed")
	}
	return nil
}
...
func (s *session) sendWillMessage() {
	...
	if err != nil {
		s.log.WithError(err).Error("Get will message failed")
	}
	if msg == nil {
		return
	}
	...
	if err != nil {
		s.log.WithError(err).Error("Retain will message failed")
	}
...
func (s *session) genSubAck(subs []packet.Subscription) []packet.QOS {
	...
	for i, sub := range subs {
		if !common.SubTopicValidate(sub.Topic) {
			...
			s.log.WithField("topic", sub.Topic).Errorf("QOS (%d) not supported", sub.QOS)
			rv[i] = packet.QOSFailure
		} else {
			rv[i] = sub.QOS
		}
