...
import (
	...
	bb "github.com/baidu/openedge/module/hub/broker"
...
func TestSessionConnect(t *testing.T) {
	...
	assert.NotNil(t, c)
	defer c.close()
	c.assertOnConnect(t.Name(), "u1", "p1", 2, packet.InvalidProtocolVersion)

	// Round 2
	c = newMockClient(t, r.sessions)
	assert.NotNil(t, c)
	defer c.close()
	c.assertOnConnect(t.Name(), "u", "p", 3, packet.BadUsernameOrPassword)
}
...
func TestSessionPub(t *testing.T) {
	...
	// Round 2
	c.assertOnPublishError("test", 2, []byte("hello"), "QOS (2) not supported")
	// Round 2
	c.assertOnPublishError("haha", 1, []byte("hello"), "Topic (haha) not permitted")
}
...
func (c *mockClient) assertOnConnect(name, u, p string, v byte, ca packet.ConnackCode) {
	conn := &packet.Connect{ClientID: name, Username: u, Password: p, Version: v}
...
func (c *mockCodec) Send(genericPacket packet.Generic, _ bool) error {
	...
	case c.out <- genericPacket:
		return nil
	case <-time.After(time.Second):
		return errors.Errorf("Send packet timeout")
	}
}
...
func (c *mockCodec) Receive() (packet.Generic, error) {
	...
	case pkt := <-c.in:
		return pkt, nil
	case <-time.After(time.Second):
		return nil, errors.Errorf("Send packet timeout")
	}
}
