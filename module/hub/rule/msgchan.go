...
var errMsgChanClosed = errors.Errorf("Message channel has been closed")
var errMsgDiscarded = errors.Errorf("Message discarded since channel is full")
...
func (c *msgchan) start() error {
	...
	return errors.Trace(c.msgtomb.Gos(c.goProcessingQ0, c.goProcessingQ1))
}

// TODO: only handle messages with qos 0 before system closed. [bce-iot-6545]
func (c *msgchan) close(force bool) {
	c.log.Debug("Message channel closing")
	c.msgtomb.Kill()
	if force {
		c.acktomb.Kill()
	}
	err := c.msgtomb.Wait()
	if err != nil {
		c.log.WithError(err).Debug("Message channel closed")
	}
	if !force {
		c.acktomb.Kill()
	}
...
func (c *msgchan) putQ0(msg *common.Message) {
	if !c.msgtomb.Alive() {
		c.log.WithError(errMsgChanClosed).Error("Put message (qos=0) failed")
		return
	}
	select {
	case c.msgq0 <- msg:
...
func (c *msgchan) putQ1(msg *common.Message) {
	...
	case <-c.msgtomb.Dying():
		c.log.WithError(errMsgChanClosed).Error("Put message (qos=1) failed")
	case c.msgq1 <- msg:
	}
}
...
func (c *msgchan) goProcessingQ0() error {
	c.log.Debug("Task to process message from channel (Q0) begins")
	defer c.log.Debug("Task to process message from channel (Q0) stopped")
loop:
	for {
		select {
		case <-c.msgtomb.Dying():
	...
	for {
		...
		case <-time.After(endTime.Sub(time.Now())):
			c.log.Warn("Process inflight messages from channel (Q0) timed out during shutdown")
			return nil
		case <-c.acktomb.Dying():
			c.log.Debug("Process inflight messages from channel (Q0) interrupted during session close")
			return nil
		default:
			c.log.Debug("Process inflight messages from channel (Q0) finished")
			return nil
		}
	}
}
...
func (c *msgchan) goProcessingQ1() error {
	c.log.Debug("Task to process message from channel (Q1) begins")
	defer c.log.Debug("Task to process message from channel (Q1) stopped")
loop:
	for {
		select {
		case <-c.msgtomb.Dying():
	...
	for {
		...
		case msg := <-c.msgq1:
			c.process(msg)
		case <-time.After(endTime.Sub(time.Now())):
			c.log.Warn("Process inflight messages (qos=1) timed out during shutdown")
			return nil
		case <-c.acktomb.Dying():
			c.log.Debug("Process inflight messages (qos=1) interrupted during session close")
			return nil
		default:
			c.log.Debug("Process inflight messages (qos=1) finished")
			return nil
		}
	}
}
...
func (c *msgchan) goWaitingAck() error {
	c.log.Debug("Task to wait acknowledge begins")
	defer c.log.Debug("Task to wait acknowledge stopped")
	for {
		select {
		case <-c.acktomb.Dying():
			return nil
...
func (c *msgchan) discard(msg *common.Message) {
	c.log.Debug(errMsgDiscarded.Error())
	if msg.QOS == 0 {
		return
	}
	msg.SetCallbackSID(c.persist)
