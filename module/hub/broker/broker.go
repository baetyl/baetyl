...
var errBrokerClosed = errors.Errorf("Broker has been closed")
...
func (b *Broker) Flow(msg *common.Message) {
	...
	if msg.QOS > 0 {
		...
		case <-b.tomb.Dying():
			b.log.Debug("Flow message (qos=0) failed since broker closed")
		}
	}
}
...
// Close closes broker
func (b *Broker) Close() {
	b.log.Info("Broker closing")
	b.tomb.Kill()
	err := b.tomb.Wait()
	b.log.WithError(err).Info("Broker closed")
}
